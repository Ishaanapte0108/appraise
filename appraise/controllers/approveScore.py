...
def approve(**kwargs):
  ...
  if is_admin:
    ...
    form_owner_dept = frappe.db.get_value("User", owner, "department")
    form_user_dept = frappe.db.get_value('User', user, "department")
  ...
  docAgg = frappe.get_doc("Aggregator", doc_name)
  docAgg.approved_score = rsc
  docAgg.approved = 1
  docAgg.save(ignore_permissions=True)
