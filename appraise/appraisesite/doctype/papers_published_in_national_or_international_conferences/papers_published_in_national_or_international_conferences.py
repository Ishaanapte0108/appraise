...
Doctype = 'Papers published in national or international conferences'
pattern_for_wtg = r'\((\s*(?:\d+\.\d+|\d+)\s*)\)'


class Paperspublishedinnationalorinternationalconferences(Document):
    """method to autoname your document"""
    def autoname(self):
        self.name = f'RB1_{self.professor}_{self.academic_year}_{self.semester}'
...
def compute_marks(self):
    ...
    for item in self.criteria_table:
        if counter > 1:
            ...
            marks_obtained = round(val1 * val2 * val3 * val4 * val5 * 50, 2)
            item.col6 = marks_obtained
