...
frappe.ui.form.on(DocType, {
  ...
  refresh: function (frm) {
    ...
    if (frm.is_new()) {
      ...
      frappe.call({
        method: 'appraise.controllers.getUserInfo.get_user_info',
        args: {
          'session_user': frappe.session.user
        },
        callback: function (response) {
          if (response) {
            frm.set_value('professor', response.message[0]);
            frm.set_value('department', response.message[1]);
            frm.set_value('designation', response.message[2]);
            frm.set_value('academic_year', response.message[3]);
            frm.set_value('semester', response.message[4]);
            frm.set_value('reviewer', response.message[5]);
          }
        }
      });
        ...
        if (frm.doc.approved == 0) {
          let d = new frappe.ui.Dialog({
            ...
            primary_action(values) {
              ...
              frappe.call({
                method: 'appraise.controllers.approveScore.approve',
                args: {
        ...
        if (frm.doc.approved == 1) {
          frappe.confirm(
            ...
            function () {
              frappe.utils.play_sound("click");
              frappe.call({
                method: 'appraise.controllers.approveScore.revoke',
                args: {
                  'session_user': frappe.session.user,
                  'owner': frm.doc.owner,
                  'doctype': frm.doc.doctype,
